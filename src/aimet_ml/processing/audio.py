...
def read_audio(
    file_path: str, target_sr: int = 16000, normalize: bool = False
    ...
    waveform = (
        np.asarray(audio.get_array_of_samples()).astype(np.float32, order="C") / 32768.0
    )
    return waveform, target_sr


def convert_audio(src_file: str, dst_file: str, target_sr: int = None) -> None:
    """
